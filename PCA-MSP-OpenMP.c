...
void bcastmvalue(int mvalue[pop][49+C]);
...
void ffunction(double fitness[pop], int mvalue[pop][49+C], int xvalue[49+C], int yvalue[49+C])
{
  ...
  {
	#pragma omp parallel for 
    for(temppop=rank*(quotient+1);temppop<=(rank+1)*(quotient+1)-1;temppop++)
    {
      fitness[temppop]=linefitness(temppop, mvalue, xvalue, yvalue);
    }
    MPI_Send(&fitness[rank*(quotient+1)],quotient+1,MPI_DOUBLE,0,0,MPI_COMM_WORLD);
  }
  else
  {
	#pragma omp parallel for 
    for(temppop=rank*quotient+remainder;temppop<=(rank+1)*quotient+remainder-1;temppop++)
    {
      fitness[temppop]=linefitness(temppop, mvalue, xvalue, yvalue);
    }
    MPI_Send(&fitness[rank*quotient+remainder],quotient,MPI_DOUBLE,0,0,MPI_COMM_WORLD);
  }
  if(rank==0)
  {
    for(i=0;i<=size-1;i++)
    {
      if(i<remainder)
      {
        MPI_Recv(&fitness[i*(quotient+1)],quotient+1,MPI_DOUBLE,i,0,MPI_COMM_WORLD,&status);
      }
      else
      {
        MPI_Recv(&fitness[i*quotient+remainder],quotient,MPI_DOUBLE,i,0,MPI_COMM_WORLD,&status);
      }
    }
  }
  MPI_Barrier(MPI_COMM_WORLD);
...
int main(int argc, char **argv)
{
  ...
  if(rank==0)
  {
    ...
    MPI_Bcast(&maxfitnessvalue,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Barrier(MPI_COMM_WORLD);
